...
Blockly.Connection.prototype.dispose = function() {
  ...
  // BlocksCAD has bad connections possible - dispose of them
...
Blockly.Connection.prototype.highlight = function() {
  ...
  if (this.type == Blockly.INPUT_VALUE || this.type == Blockly.OUTPUT_VALUE) {
    ...
    if (this.sourceBlock_.RTL) {
      steps = 'm 25,0 h -10 ' + Blockly.BlockSvg.NOTCH_PATH_RIGHT + ' h -10';
    } else {
      steps = 'm -25,0 h 10 ' + Blockly.BlockSvg.NOTCH_PATH_LEFT + ' h 10';
    }
  }
...
// for BlocksCAD, also check for illegal connections so we can highlight them as illegal
...
Blockly.Connection.prototype.closest = function(maxLimit, dx, dy) {
  ...
  // we want to know if a connection's type is legal OR NOT for BlocksCAD.
  var legal = true;
   ...
   */
   ...
   // changed for BlocksCAD: I don't return true for an incompatible connection.  I 
   // just set the "legal" field to false.  
  ...
  function checkConnection_(yIndex) {
    ...
    // These four lines are commented out for BlocksCAD.
    // // Do type checking.
    // if (good && !thisConnection.checkType_(connection)) {
    //   return true;
    // } 
    ...
    if (r <= maxLimit) {
      //  set legal to false for BlocksCAD
         // Do type checking.
  ...
  return {connection: closestConnection, radius: maxLimit, allowed: legal};
};
