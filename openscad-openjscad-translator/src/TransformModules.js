define("TransformModules", ["Globals", "Context"], function(Globals, Context){
    ...
    ExtrudeTransform.prototype.evaluate = function(parentContext, inst){
        ...
        var scale = Context.contextVariableLookup(context, "scale", 1);
    ...
    function RotateExtrudeTransform(a){
        TransformModule.call(this, a);
    };

    RotateExtrudeTransform.prototype.evaluate = function(parentContext, inst){
        inst.argvalues = [];
        ...
        var context = Context.newContext(parentContext, ["$fn", "$fs", "$fa", "faces"], [], inst);

        var fn = Context.contextVariableLookup(context, "$fn", Globals.FN_DEFAULT);
        var fs = Context.contextVariableLookup(context, "$fs", Globals.FS_DEFAULT);
        var fa = Context.contextVariableLookup(context, "$fa", Globals.FA_DEFAULT);

        // can I add a twist?  JY
        var twist = Context.contextVariableLookup(context, "twist", 0);
        var radius = Context.contextVariableLookup(context, "radius", 0);
        var tsteps = Context.contextVariableLookup(context, "tsteps", 0);

        //var faces = Context.contextVariableLookup(context, "$fn", 8);

        return this.transformChildren(inst.children, context, function(){
            var template = _.template(".rotate_extrude({faces: <%=fn%>,radius: <%=radius%>,tsteps: <%=tsteps%>,twist: <%=twist%>})", {fn:fn, radius:radius, tsteps:tsteps, twist:twist});
            return template;
        });
    };
	...
	return {
        ...
        RotateExtrude: RotateExtrudeTransform
	}
});